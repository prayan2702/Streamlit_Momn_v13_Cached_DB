...
Changes vs v13:
  • UI v14 redesign — all logic/calculations untouched
  • New design system: Plus Jakarta Sans + JetBrains Mono fonts
  • Cleaner, minimal fintech palette (Zerodha/Smallcase-inspired)
  • App header: borderless bar with subtle separator, pill badge
  • Step bar: dot-based progress with connecting track line
  • Metric cards: flat, number-forward, hairline top accent
  • Section headers: typographic-only, no heavy borders
  • Chips: compact, high-contrast pill labels
  • Reb-strip: zebra-stripe table card
  • Login: centered glassmorph card, cleaner brand mark
  • Sidebar: minimal, tighter spacing, no emoji dot clutter
"""
...
.mono { font-family: 'JetBrains Mono', 'Fira Code', monospace; letter-spacing: -.2px; }
...
.app-title .brand { color: var(--teal); }
.app-title .divider {
    width: 1px;
    height: 16px;
...
.active .step-circle { background: var(--indigo); color: #fff; box-shadow: 0 0 0 4px rgba(79,70,229,.15); }
...
.section-hdr.red::before    { background: var(--red); }
.section-hdr.green::before  { background: var(--green); }
.section-hdr.violet::before { background: var(--violet); }
.section-hdr.blue::before   { background: var(--blue); }
...
.nse-link-box .hint { font-size: 11.5px; color: var(--muted); margin-top: 2px; }
...
.reb-stat:hover { background: var(--bg); }
...
.qlink-btn:hover { opacity: .92; transform: translateY(-2px); box-shadow: var(--shadow-md); }
.qlink-rebalancer {
    background: #1e40af;
...
def login_page():
    ...
    with mid:
        ...
        <div class="login-card">
          ...
          <div class="login-sub">NSE Momentum Strategy &nbsp;·&nbsp; v14</div>
        </div>
...
<div class="app-header">
  <div style="display:flex;align-items:center;gap:10px;">
    ...
    <div>
      <div class="app-title">
        <span class="brand">Momn</span> Screener
        <span class="divider"></span>
        <span style="font-weight:500;color:var(--text-sub);font-size:14px;">Portfolio Rebalancer</span>
  ...
  <div class="app-header-right">
    <span class="header-date">📅 {_today_str}</span>
    <div class="user-tag">👤 prayan2702</div>
  </div>
</div>
""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    for s, lbl in step_labels.items():
        ...
        label_text = f"{status_icon} {step_icons[s]} {s}. {lbl}"
    ...
    <div style="margin-top:12px;padding:10px 12px;background:var(--bg);border:1px solid var(--border);
                border-radius:var(--radius-md);text-align:center;font-size:10px;color:var(--text-muted);">
    <span style="color:var(--teal);font-weight:700;letter-spacing:.3px;">MOMN SCREENER v14</span><br>
    <span>📅 {datetime.date.today().strftime('%d %b %Y')}</span>
    </div>
    """, unsafe_allow_html=True)
...
if st.session_state.current_step == 1:
    ...
    with c2:
        ...
        st.markdown(f"""
        <div style="background:var(--bg-white);border:1px solid var(--border);border-top:3px solid var(--teal);
          ...
          <span style="font-size:26px;">{_m[1]}</span>
          <div>
            <div style="font-size:15px;font-weight:800;color:var(--text-main);letter-spacing:-.3px;">{chosen_u}</div>
            <div style="font-size:12px;color:var(--text-muted);margin-top:3px;">
              <span style="font-weight:700;color:var(--teal);font-family:'JetBrains Mono',monospace;">{_m[0]}</span>
              <span style="margin:0 6px;color:var(--border)">|</span>{_m[2]}
            </div>
          </div>
        </div>
        """, unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════════════
    ...
    with col_info:
        ...
        <div style="background:var(--bg-white);border:1px solid var(--border);border-radius:var(--radius-md);
                    padding:12px 18px;font-size:12.5px;color:var(--text-sub);line-height:2.2;
                    box-shadow:var(--shadow-xs);">
        🌐 Universe: <b style="color:var(--text-main)">{U}</b> &nbsp;·&nbsp;
        📋 Symbols: <b style="color:var(--indigo);font-family:'JetBrains Mono',monospace;">{n_loaded}</b> &nbsp;·&nbsp;
...
# ═══════════════════════════════════════════════════════════════
    ...
    if portfolio and st.session_state.dfFiltered is not None:
        ...
        with col_sell:
            st.markdown('<div class="section-hdr red">🔴 SELL List</div>', unsafe_allow_html=True)
        ...
        with col_buy:
            st.markdown('<div class="section-hdr green">🟢 BUY List (New Entry)</div>', unsafe_allow_html=True)
        ...
        with col_hold:
            st.markdown('<div class="section-hdr violet">🔵 HOLD (Retain)</div>', unsafe_allow_html=True)
